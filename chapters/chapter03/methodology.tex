...
The research adopts a structured experimental design comprising five sequential phases: (1)~dataset selection and acquisition, (2)~data preprocessing and feature engineering, (3)~model design and training, (4)~comparative evaluation, and (5)~interpretability analysis and system design. Each phase builds on the outputs of the preceding one, and the overall pipeline is designed to be reproducible: all preprocessing parameters, model architectures, hyperparameter search grids, random seeds, library versions, and evaluation protocols are fully specified (see Appendix~\ref{app:supplementary}) to enable replication by independent researchers. Reproducibility is treated not as an aesthetic preference but as an epistemic requirement: claims about the relative performance of algorithms are only meaningful when the experimental conditions under which they were produced can be exactly reconstituted by a third party.
...
The selection of the primary experimental dataset was guided by four criteria: (1)~\textbf{realism}---the dataset should reflect authentic network traffic patterns rather than purely synthetic constructs, including realistic background traffic from common applications such as web browsing, email, streaming, and file transfer; (2)~\textbf{diversity}---the dataset should cover a broad range of contemporary attack types, including both volumetric (DDoS, DoS) and application-layer (SQL injection, XSS, command injection, brute force, botnet C\&C) attacks, as well as reconnaissance activity (port scanning); (3)~\textbf{richness}---the feature set should be sufficient for meaningful ML-based analysis without requiring extensive manual feature engineering, ideally including flow-level statistical descriptors such as packet rates, byte rates, inter-arrival times, and TCP flag counts; and (4)~\textbf{benchmarkability}---the dataset should be widely used in the literature to enable direct comparison with prior work, with a clear licensing regime permitting academic and reproducible use.

\subsection{CICIDS2017 Dataset Description}

Based on these criteria, CICIDS2017~\cite{sharafaldin2018cicids} was selected as the primary data source. The alternative candidates---NSL-KDD, UNSW-NB15, and CIC-IDS2018---were eliminated for reasons set out in Chapter~\ref{chap:literature}: NSL-KDD is too dated, UNSW-NB15 was generated under fully synthetic conditions using the IXIA PerfectStorm framework, and CIC-IDS2018, while larger, introduces additional engineering complexity (16~million records across multiple AWS regions) that is not justified by the marginal informational gain for this study.

The CICIDS2017 dataset was generated over five days (3--7~July~2017) at the Canadian Institute for Cybersecurity, using CICFlowMeter to extract flow-level features from raw packet captures. It contains approximately 2.8~million flow records with 80 pre-extracted features per record, organised into seven daily capture files (one per business day). Each flow corresponds to a bidirectional TCP, UDP, or ICMP conversation between two endpoints, identified by the 5-tuple (source IP, destination IP, source port, destination port, transport protocol) and bounded by activity timeout.
...
The seven individual CSV files provided with CICIDS2017 were consolidated into a unified DataFrame using Apache Pandas, with column types coerced to fixed numeric and categorical types to ensure consistency across days. Total memory footprint after consolidation was approximately 2.1~GB. Initial inspection revealed the following data quality issues:
\begin{itemize}
...
All numerical features were standardised using z-score normalisation. The transformation parameters $\mu$ and $\sigma$ were estimated exclusively from the training partition and then applied unchanged to the validation and test partitions, in line with standard practice to avoid information leakage from held-out data into the model. The transformation is defined as:
\begin{equation}
z_i = \frac{x_i - \mu}{\sigma}
\label{eq:zscore}
\end{equation}
where $x_i$ is the raw feature value, $\mu$ is the sample mean, and $\sigma$ is the sample standard deviation computed on the training set. This transformation ensures that all features have zero mean and unit variance, preventing features with large absolute magnitudes (such as raw byte counts that can span six orders of magnitude) from disproportionately influencing distance-based algorithms (KNN), kernel-based algorithms (SVM), and gradient-based algorithms (MLP). Tree-based algorithms such as Random Forest, XGBoost, and LightGBM are scale-invariant in principle, but the standardised features were used uniformly across all six models for consistency and to enable direct comparison of feature-importance magnitudes.
...
The Synthetic Minority Oversampling Technique (SMOTE), introduced by Chawla et al.\ in 2002, was applied to augment underrepresented attack classes. SMOTE was preferred over naive random oversampling because the latter merely duplicates existing minority examples and offers no protection against overfitting to specific outlier flows. For each minority-class sample $x_i$, SMOTE generates synthetic instances by interpolating with $k$ nearest neighbours in feature space:
\begin{equation}
